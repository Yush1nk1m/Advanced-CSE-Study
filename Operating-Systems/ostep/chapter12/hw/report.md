...
$U$ = (첫 번째로 끝난 작업의 종료 시점) / (두 번째로 끝난 작업의 종료 시점) 이다. 위 경우 평균적으로 $U$는 1에 매우 근접한 값을 가지므로 공정하다고 할 수 있다.

## Problem 4

    타임 퀀텀의 크기 (-q)가 커질수록 이전 질문에 대한 대답은 어떻게 바뀔까?

타임 퀀텀의 크기가 커지면 첫 번째로 끝난 작업의 종료 시점과 두 번째로 끝난 작업의 종료 시점 간 차이의 편차가 커지기 때문에 $U$ 의 값은 0에 가까워진다. 예를 들어 3번 문제의 상황에서 타임 퀀텀이 100으로 늘어난다면 $U$ 의 값은 항상 0.5로, 이전 문제에서 1에 매우 근접한 값을 가졌을 때와 달리 상당히 불공정해진 것을 확인할 수 있다.
